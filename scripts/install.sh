...
if [[ ! -s "$APP_ROOT/config/app.env" ]]; then
  if [[ -f "$APP_ROOT/app/.env.example" ]]; then
    cp "$APP_ROOT/app/.env.example" "$APP_ROOT/config/app.env"
    log_success "Default config written from .env.example"
  else
    log_info "No .env.example found — config will be initialized on first start."
  fi
else
...
if [[ ! -s "$APP_ROOT/config/config.json" ]]; then
  echo '{"telemetry":false,"maintenance_mode":false,"update_channel":"stable"}' > "$APP_ROOT/config/config.json"
fi
